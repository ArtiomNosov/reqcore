...
const DEMO_ORG_NAME = DEMO_ORG_NAME_RU
...
const JOBS_DATA = [...JOBS_DATA_RU]

const CANDIDATES_DATA = CANDIDATES_DATA_RU

const FULLSTACK_QUESTIONS = FULLSTACK_QUESTIONS_RU
const DESIGNER_QUESTIONS = DESIGNER_QUESTIONS_RU
const DEVOPS_QUESTIONS = DEVOPS_QUESTIONS_RU
...
function localizeAssignments(apps: ApplicationAssignment[]): ApplicationAssignment[] {
  return apps.map((a) => ({
    ...a,
    notes: a.notes ? (ruAppNote(a.status, a.score) ?? a.notes) : a.notes,
  }))
}
...
const JOB_APPLICATIONS = [
  localizeAssignments(JOB_0_APPS),
  localizeAssignments(JOB_1_APPS),
  localizeAssignments(JOB_2_APPS),
  localizeAssignments(JOB_3_APPS),
  localizeAssignments(JOB_4_APPS),
]
...
function generateResponses(jobIndex: number, candidateIndex: number): Record<string, string | string[] | boolean> {
  ...
  if (jobIndex === 0) {
    ...
    const starts = ['Сразу', '2 недели', '1 месяц', '2 недели', '1 месяц', '2–3 месяца']
    const problems = [
      'Мигрировали монолитный REST на GraphQL federation без простоя для 12 микросервисов.',
      'Реализовали real-time редактор на CRDT с 500+ одновременными пользователями.',
      'Перестроили схему БД под мультитенантность: −60% latency, −40% storage.',
      'Автоматизировали canary-деплой: −75% инцидентов в проде.',
      'Собрали state management для формы с 20+ условными шагами и offline.',
      'Генератор type-safe API-клиента из OpenAPI убрал целый класс runtime-ошибок.',
    ]
    ...
    return {
      'Стаж работы с TypeScript': year,
      'Предпочитаемый frontend-фреймворк': framework,
      'Опишите сложную техническую задачу, которую вы недавно решили': problem,
      'Ссылка на GitHub или портфолио': `https://github.com/${candidate.firstName.toLowerCase()}${candidate.lastName.toLowerCase().replace(/['\s]/g, '')}`,
      'Когда можете выйти на работу?': start,
    }
  }
  if (jobIndex === 1) {
    const tools = ['Figma', 'Figma', 'Sketch', 'Figma']
    const processes = [
      'Интервью со стейкхолдерами → исследование → wireframes в FigJam → high-fi в Figma с библиотекой компонентов.',
      'Research → конкуренты → user flows → прототипы → тесты → итерации. Всегда проверяю с пользователями до разработки.',
      'Double diamond: Discover → Define → Develop → Deliver. Всё фиксирую в design spec для handoff.',
      'Быстрые скетчи → Figma → guerrilla-тесты → итерации. Скорость обучения важнее идеала.',
    ]
    ...
    return {
      'Ссылка на портфолио': `https://dribbble.com/${candidate.firstName.toLowerCase()}${candidate.lastName.toLowerCase().charAt(0)}`,
      'Основной инструмент дизайна': tool,
      'Опишите процесс работы над недавним проектом': process,
      'Есть опыт с дизайн-системами': candidateIndex % 2 === 0,
    }
  }
  if (jobIndex === 2) {
    ...
    return {
      'С какими облаками работали?': platformList,
      'Стаж работы с Docker': dockerYear,
      'Предпочитаемая CI/CD платформа': ciPlatform,
    }
  }
  return {}
}
